...
with open('twitter_network.csv', 'r', encoding="latin-1", errors="backslashreplace") as inputfile:
        twitter_network = [line.strip().split('\t') for line in inputfile]
...
for nodeset_typename, nodeset_test in nodeset_types.items():
...
for k in nodesets.keys():
    ns = [ hfollowers[n] * 5 for n in nodesets[k] ] 
