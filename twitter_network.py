...
for f in glob.glob('twitter-users\*.json'):
    with open(f, 'r', encoding="latin-1", errors="backslashreplace") as jasondata:
        data = json.load(jasondata)
...
def process_follower_list(screen_name, edges=[], depth=0, max_depth=2):
    ...
    with open(f, 'r', encoding="latin-1", errors="backslashreplace") as inputfile:
        followers = [line.strip().split('\t') for line in inputfile]
